...
For the live demo, treat the prototype dataset as a replayable IoT sensor stream. The goal is to make the system behave as if live readings are arriving from a facility every few minutes.

Recommended replay source:

```text
data/processed/operational_compliance_dataset.csv
```

This file is better for the live dashboard than the engineered model matrix because it represents operational sensor/lab/soft-sensor readings. The backend should replay it row by row in timestamp order.

Do not use this as the main live feed unless the team wants a shortcut-only test:

```text
data/processed/preprocessed_model_matrix.csv
```

That file is generated by Notebook 03 and already contains engineered features. It is useful for model debugging, but it is less realistic for a real-time IoT demo because a production system receives raw/current readings first and then computes features from recent history.
...
- Send one row every 1-5 seconds in demo time.
- Each row can still represent the original 5-minute sensor interval.
- Keep a rolling history buffer of at least the previous 2 hours because preprocessing uses lag/rolling features up to 120 minutes.
- During the warm-up period, show `INITIALIZING` or `INSUFFICIENT_HISTORY` until enough history exists for full features.

Feature builder contract:

- The backend should not pass one raw row directly to the model.
- It should append the new row to the rolling history buffer.
- It should compute the same lag, rolling, delta, rate-of-change, and time features defined by Notebook 03.
- It should apply the training medians from `data/processed/preprocessing_train_medians.csv` for missing model inputs.
- It should order columns exactly as `metadata["feature_columns"]` from `services/ml/models/aquasense_best_models.joblib`.
- It must not use audit-only fields such as `breach_episode_id`, `incident_group_id`, or `minutes_to_next_breach` as model inputs.

Files the backend should read:
...
data/processed/model_selection_summary.json
data/processed/model_card_summary.json
data/processed/operational_compliance_dataset.csv
```

The backend should load the model bundle once at startup:

```python
import joblib

bundle = joblib.load("services/ml/models/aquasense_best_models.joblib")
feature_columns = bundle["metadata"]["feature_columns"]
breach_classifier = bundle["breach_classifier"]
...
For each replayed sensor row, the backend should produce one dashboard event with current readings, forecasts, risk, anomaly status, and final alert level.
...
```json
{
  ...
  "breach_risk_30min": 0.72,
  "anomaly_flag": false,
  "alert_reason": "COD forecast is approaching the configured discharge limit.",
...
| Status | Trigger |
|---|---|
| `RED` | Current deterministic rule breach. Current value is already outside a configured consent/permit limit. |
| `AMBER` | No current breach, but +30 minute forecast crosses a configured limit or classifier risk exceeds its selected threshold. |
| `WATCH` | Current and forecast values are still compliant, but risk is rising, margin to limit is narrowing, or anomaly/data-quality warning exists. |
| `GREEN` | Current values compliant, forecasts remain below limits, and no anomaly/data-quality issue is active. |

Recommended WebSocket topic names:

```text
sensor.reading
compliance.current
prediction.breach_risk
prediction.forecast_30min
anomaly.sensor_quality
alert.status
```

This lets the frontend animate a realistic facility timeline: normal operation, early warning, predicted breach risk, and confirmed rule breach if current values cross limits.
